...
func (m *Status) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.HealthRollup != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.HealthRollup))
		i--
		dAtA[i] = 0x18
	}
	if m.Health != 0 {
...
func (m *Status) SizeVT() (n int) {
	...
	if m.HealthRollup != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.HealthRollup))
	}
...
func (m *Status) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field HealthRollup", wireType)
			}
			m.HealthRollup = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.HealthRollup |= Health(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
